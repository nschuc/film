...
def main(args):
  ...
  if args.question_file is not None and args.image is not None:
    feats_var = extract_image_features(args, dtype)
    with open(args.question_file, 'r') as f:
      for q in f.readlines():
        print(q)
        run_single_example(args, model, dtype, q.strip(), feats_var)
  # Interactive mode
...
def run_single_example(args, model, dtype, question_raw, feats_var=None):
  ...
  if args.distill_viz:
    viz_dir = os.path.join(args.output_viz_dir, 'norms')
    if not os.path.isdir(viz_dir):
      os.mkdir(viz_dir)
    args.viz_dir = viz_dir

    print('Saving visualizations to ' + args.viz_dir)
    film_params = predicted_program.cpu().data.numpy()
    norms = get_norms(torch.stack(ee.module_outputs, dim=1))
    # Visualize resblock featuremaps
    data = {
      'question': question_raw,
      'channel_norms': { f'residual_{i}': n for i, n in enumerate(norms) },
      'film_params': film_params.tolist()
    } 
    import json
    save_file = os.path.join(args.viz_dir, question_raw + '_norms.json')
...
def get_norms(features):
    ...
    norms = fmap_norms.data.squeeze().tolist()
    return norms
